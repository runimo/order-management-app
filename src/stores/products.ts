...
export const useProductsStore = defineStore('products', {
  ...
  actions: {
    ...
    updateProduct(product: ProductDraft | null): void {
      if (product) {
  ...
  getters: {
    ...
    productsByCompanyId(state) {
      return (companyId: string): Product[] => {
        return state.products.filter(
          (product) => product.relationships.supplier.data.id === companyId,
        )
      }
    },
    ...
    productsByCurrentUserCompany(state): Product[] {
      ...
      return state.products.filter(
        (product) => product.relationships?.supplier?.data?.id === userStore.currentUser.company.id,
      )
    },
  },
})
