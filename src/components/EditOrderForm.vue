...
import type { Order, OrderTransformedForEdit, Product } from '@/types/index.d.ts'
...
const { resetOrderByType, updateOrder } = ordersStore
...
const orderToEdit = ref<OrderTransformedForEdit | null>(null)
const setOrderToEdit = () => {
  if (order) {
    const { createdAt, orderNumber, quantities, status, totalPrice } = order.attributes
    const products = quantities.map((product) => {
      const { productId, count } = product
      const fullProduct = productsStore.productById(productId) as Product
...
<template>
  <div v-if="orderToEdit" class="flex flex-col justify-center space-y-4 mt-8">
    <div class="max-w-full p-4 bg-white rounded-xs shadow border border-gray-200">
      ...
      <div class="grid gap-1">
        ...
        <div class="flex justify-between">
          ...
          <span>
            {{
              companiesStore.companies.find((company) => company.id === orderToEdit?.supplierId)
                ?.attributes.name
            }}
          </span>
        </div>
      ...
      <div class="flex flex-col sm:flex-row justify-end w-full gap-2 mt-6">
        <BaseButton class="w-full sm:w-[30%]" variant="secondary" @click="cancel">
          Cancel
        </BaseButton>
        <BaseButton class="w-full sm:w-[30%]" @click="handleSaveEdit"> Save </BaseButton>
      </div>
    </div>
  </div>
</template>
